...
public class ModelImpl implements Model {
  ...
  public boolean isLit(int r, int c) {
    ...
    while (rRight < puzzle.getHeight()) {
      if (puzzle.getCellType(rRight, c) != CellType.WALL
          && puzzle.getCellType(rRight, c) != CellType.CLUE) {
        if (isLamp(rRight, c)) {
          flag = true;
          break;
        }
    ...
    int rLeft = r - 1;
    while (rLeft >= 0) {
      if (puzzle.getCellType(rLeft, c) != CellType.WALL
          && puzzle.getCellType(rLeft, c) != CellType.CLUE) {
        if (isLamp(rLeft, c)) {
          flag = true;
          break;
        }
    ...
    while (cUp < puzzle.getWidth()) {
      if (puzzle.getCellType(r, cUp) != CellType.WALL
          && puzzle.getCellType(r, cUp) != CellType.CLUE) {
        if (isLamp(r, cUp)) {
          flag = true;
          break;
        }
    ...
    int cDown = c - 1;
    while (cDown >= 0) {
      if (puzzle.getCellType(r, cDown) != CellType.WALL
          && puzzle.getCellType(r, cDown) != CellType.CLUE) {
        if (isLamp(r, cDown)) {
          flag = true;
          break;
        }
    ...
    for (int k = 0; k < puzzle.getHeight(); k++) {
      for (int j = 0; j < puzzle.getWidth(); j++) {
        if (puzzle.getCellType(k, j) == CellType.CORRIDOR && !isLit(k, j)) {
          ...
          return false;
        } else if (puzzle.getCellType(k, j) == CellType.CORRIDOR
            && isLamp(k, j)
            && isLampIllegal(k, j)) {
          return false;
        }
